...
export async function getBCHTransactionDetail(txid) {
  try {
      const res = await callOKLinkApi('get', '/api/v5/explorer/transaction/transaction-fills', {
          chainShortName: 'BCH',
          txid
      });
...
export function getBCHWithdrawalFee(utxos, feeRate, amount, splitGranularity) {
  const fee = nerve.bch.calcFeeWithdrawal(utxos, amount, feeRate, splitGranularity);
