...
export default function useEthereum() {
  ...
  function initTronChainInfo(provider: any, address: string) {
    ...
    walletStore.changeAddress(address)
    walletStore.changeNetwork('TRON')
  }
  ...
  async function initBTCChainInfo(provider: any, address: string) {
    ...
    walletStore.changeIsWrongChain(isWrongChain)
    walletStore.changeAddress(address)
    walletStore.changeNetwork('BTC')
    if (isWrongChain) {
      switchBTCNetwork()
    }
  }

  function initFCHChainInfo(address: string, network: string) {
    /* store.commit('changeIsWrongChain', false)
    store.commit('changeAddress', address)
    store.commit('changeNetwork', network) */
    walletStore.changeIsWrongChain(false)
    walletStore.changeAddress(address)
    walletStore.changeNetwork(network)
  }
  ...
  async function initEVMChainInfo(provider: any, address: string) {
    ...
    store.commit('changeNetwork', network) */
    console.log(isWrongChain, '-0-0-0-');
    walletStore.changeIsWrongChain(isWrongChain)
    walletStore.changeAddress(currentAddress)
    walletStore.changeNetwork(network)
  }
    ...
    if (provider.isNabox && (N_CHAINS.includes(network))) {
      ...
      try {
        ...
        if (chainInfo && network !== chainInfo.name) {
          ...
          walletStore.changeNetwork(chainInfo.name)
        }
      } catch (e) {
        //
      }
      // store.commit('changeIsWrongChain', !validAddress)
      walletStore.changeIsWrongChain(!validAddress)
      if (validAddress) {
        if (
          state.address &&
  ...
  function handleChainChange(chainId: string) {
    ...
    if (chainId && chainId !== oldChainId) {
      ...
      // store.commit('changeNetwork', network)
      walletStore.changeNetwork(network)
      walletStore.changeIsWrongChain(!network)
      state.chainId = chainId
  ...
  function disconnect() {
    ...
    storage.remove('providerName')
    storage.remove('network')
    storage.remove('assetList', 'session')
    state.address = ''
    reload()
  }
