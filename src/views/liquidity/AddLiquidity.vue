<template>
  <div class="add-liquidity" v-loading="state.loading">
    ...
    <div
      class="my-5 rounded-xl bg-card2 p-4 text-center text-primary"
      v-show="state.insufficient">
    ...
    <custom-input
      ...
      @max="max('to')"></custom-input>

    <div class="liquidity-info pt-7" v-if="priceInfo">
      <div class="mb-2 text-label">{{ $t('liquidity.liquidity8') }}</div>
      <div class="content flex items-center rounded-xl bg-input px-4 py-6">
        <div class="flex flex-1 flex-col items-center">
          <div class="mb-2 leading-4">{{ priceInfo.from_to }}</div>
          <p class="text-xs leading-4 text-label">
            {{ state.toAsset && state.toAsset.symbol }} Per
            {{ state.fromAsset && state.fromAsset.symbol }}
          </p>
        </div>
        <div
          class="flex flex-1 flex-col items-center border-x border-[#292C36]">
...
async function createPair() {
  ...
  if (fromAsset && toAsset) {
    ...
    try {
      ...
      handleResult(61, res, fromAsset.symbol + '-' + toAsset.symbol)
      if (res && res.hash) {
...
async function addLiquidity() {
  ...
  if (fromAsset && toAsset) {
    ...
    try {
      ...
      const amountRemark = `${toThousands(fromAmount)} ${fromAsset.symbol} + ${toThousands(toAmount)} ${toAsset.symbol}`
      handleResult(64, res, amountRemark)
